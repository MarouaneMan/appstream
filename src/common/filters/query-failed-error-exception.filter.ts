import {ArgumentsHost, BadRequestException, Catch, ExceptionFilter, InternalServerErrorException, Logger} from '@nestjs/common';
import { QueryFailedError } from 'typeorm';

/**
 * SQL Violation constants
 */
...
export class QueryFailedErrorExceptionFilter implements ExceptionFilter {
  ...
  handleUniqueConstraintViolation(exception: QueryFailedError) {
    ...
    const re = /^Key \((.+?)\)/;
    const match = exception.driverError.detail.match(re);

    // Should never happen
    if (!match) throw new InternalServerErrorException();
